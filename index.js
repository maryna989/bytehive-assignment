...
`,c=document.querySelector("#header");document.querySelector(".js-logo");c.insertAdjacentHTML("afterbegin",d);c.insertAdjacentHTML("beforeend",f);const m=[{title:"People",value:"200"},{title:"Our users the world",value:"50"},{title:"Succesfull projects",value:"100"}],v=m.map((t,s)=>{let r=t.value;return s===0?r=`${r}+`:s===1?r=`${r}K`:s===2&&(r=`${r}%`),`<li class="static-item"><h3>${t.title}</h3><p class="item-value">${r}</p></li>`}).join(""),g=`<ul class="statistics-list">${v}</ul>`,p=document.querySelector(".js-statistics");document.querySelector("#js-statis");p.insertAdjacentHTML("beforeend",g);const h=[["Creative Direction","Art Direction","Visual Design","Service Design"],["UX Design","Brand Identity","Concepting","Interaction Design"],["Design Systems","Strategy","Digital Experiences","Consulting"]],y=()=>`
      <div class="our-services">
        ${h.map(t=>`
          <div class="services-group">
            ${t.map(s=>`<a href="#" class="service-item">${s}<svg class="icon-arrow" width="13" height="13"><use href='../img/icons.svg#icon-arrow'></use></svg></a>`).join("")}
          </div>
        `).join("")}
      </div>
